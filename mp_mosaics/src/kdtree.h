...
class KDTree
{
  ...
  public:
    ...
    unsigned Partition(vector<Point<Dim>>& newPoints,unsigned left,unsigned right,unsigned ind,int dim);
    Point<Dim> QuickSelect(vector<Point<Dim>>& newPoints,unsigned left,unsigned right,unsigned ind,int dim);
    ...
    KDTree<Dim>::KDTreeNode* TreeGrow(vector<Point<Dim>>& newPoints,unsigned left, unsigned right, int dim);
    ...
    KDTree<Dim>::KDTreeNode* _copy(KDTreeNode* otherroot);
    ...
    Point<Dim> dfs(KDTreeNode* subroot, const Point<Dim>& query,int dim) const;
